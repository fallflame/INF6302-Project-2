...
public class TP2Visitor_v4_backup implements JavaParser1_7Visitor {
    ...
    class Relation{
    	...
    	public boolean equals(Relation r){
			return (this.fromNode == r.fromNode 
					&& this.toNode == r.toNode 
    ...
    /*
    private void removeDuplicatedRelations(){
    	ArrayList<Relation> newList = new ArrayList<>();
    	
    	for( Relation r1: relationList){
    		Boolean already = false;
    		for (Relation r2 : newList){
    			if (r1.equals(r2)){
    				already = true;
    				break;
    			}
    ...
    private void printResult(){
    	...
    	for (ClassNode cn : TP2Visitor_v4_backup.classNodeList){
    		cn.printClassNode();
    	...
    	relationListString = "";
    	for (Relation r : TP2Visitor_v4_backup.relationList){
    ...
    public Object visit(NormalClassDeclaration node, Object data){
    	...
    	for (int i = 0; i < node.jjtGetNumChildren(); i++){
            ...
            if (childNode.toString() == "Type"){
            	...
            	TP2Visitor_v4_backup.relationList.add(new Relation(className, getFirstToken(childNode), "generalization"));
                break;
            }
        }
        ...
        TP2Visitor_v4_backup.classNodeList.add(classNode);
...
/****************************************** Field ************************************************/
    public Object visit(FieldDeclaratorsRest node, Object data){
    	...
    	if (node.jjtGetParent().jjtGetParent().jjtGetChild(0).jjtGetChild(0).toString() != "BasicType"){ // if it is not a BasicType, add a relationship
    		...
    		TP2Visitor_v4_backup.relationList.add(new Relation(type, classNodeStack.peek().className, "composition"));
    	}
